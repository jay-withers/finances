...
`azurerm` and would need the `azapi` provider, which no repo in this estate
uses. Shared also suits the data, which is a household's rather than a person's
— there is nothing here one member of the household should see and the other
should not, so per-user identity would add a sign-in without adding a boundary.

Copied from jay-withers/gym-log src/gymlog/api/deps.py, unchanged but for the
cookie name. Fix bugs in both.
...
def _signing_key() -> bytes:
    ...
    every scale-from-zero — would silently log the phone out again. At
    `min_replicas = 0` and an app opened twice a month, that is every single
    visit.
    """
...
def check_passcode(presented: str) -> bool:
    ...
    refusal — which, for an app about pounds sterling, is not a hypothetical.
    Encoding both sides first is also what makes the comparison well-defined:
    two strings are equal here only if their UTF-8 is.
    """
...
def require_session(finances_session: str | None = Cookie(default=None)) -> None:
