...
Copied from jay-withers/gym-log src/gymlog/telemetry.py, which took it from
repo-agent, which took it from market-agent. Re-copy rather than diverge; fix
bugs in all four. Only the logger and service names differ.
...
- **Only the `finances` logger is exported.** The distro attaches its handler to
...
- **Performance counters and live metrics are off.** Both are periodic emissions
  from an app that is scaled to zero almost all of the time, and Container Apps
  already reports CPU and memory as free platform metrics.
...
every span in the workspace and would say nothing about the household's money.
"""
...
def configure(role: str) -> bool:
    ...
    **Must run before the FastAPI application is imported.** The instrumentation
    ...
    # calculations and the tests need neither the SDK nor a connection string —
    # the same reason settings.py defers its Azure imports.
