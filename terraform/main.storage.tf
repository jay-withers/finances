# Where the finances document lives.
#
# One small JSON document, read whole on each request and written whole on each
# change. That access pattern is why this is a blob rather than a table: there
# is no query to serve. A payday run adds nine ledger entries, a renewal or a
# wealth snapshot adds one row — a few hundred rows a year, well under 100 KB.
#
...
resource "azurerm_storage_account" "state" {
  # **`prevent_destroy` is set.** This document is the product: years of pot
  # balances, renewals and wealth snapshots that exist nowhere else once the
  # spreadsheet is gone, and that no subsequent run reconstructs. Destroying the
  # rest of this stack should not take it with it.
  #
  ...
  # `.name_unique`, not `.name`: storage account names are DNS-based and
  # globally unique across every Azure tenant, not just this subscription, so
  # `stfinancesdev` collides with whoever else in the world picked that name
  # first — it did, on the first apply. See the identical note on the Key
  # Vault above.
  ...
  blob_properties {
    ...
    # otherwise have: a payday run against the wrong month, or an amount typed
...
resource "azurerm_storage_container" "state" {
  ...
  #   Log Analytics workspace, whose `daily_quota_gb` is split across every
...
# The container app and the digest job read and write the document through this.
#
...
# Whoever applies can read and edit the document by hand — which is what
# `finances import` and `finances show` do, running locally against the real
# blob.
