...
| <a name="input_digest_cron_expression"></a> [digest\_cron\_expression](#input\_digest\_cron\_expression) | When the reminder digest runs, in **UTC** — Container Apps jobs have no timezone setting. The default is 17:00 UTC every Friday: early evening UK time either side of the clock change (17:00 GMT, 18:00 BST), so anything that needs attention has a chance to surface before the weekend rather than waiting for a fixed date. | `string` | `"0 17 * * 5"` | no |
